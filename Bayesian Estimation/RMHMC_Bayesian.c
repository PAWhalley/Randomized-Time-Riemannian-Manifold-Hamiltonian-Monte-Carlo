...
void one_RATTLE_step(double q[n], double v[n], double f[n], double data[d][m], double mean[d])
{
...
void one_step_HMC(int num_leap, double qold[n], double pold[n], double q[n], double v[n], double f[n], double data[d][m], double mean[d])
{
    ...
    {
    one_RATTLE_step(q,v,f,data,mean);
    }
    //M-H step
...
int main(int argc, const char * argv[]) {
    ...
    for (num_leap = 1; num_leap < 50; num_leap++){
        ...
        { 
            one_step_HMC(10*num_leap,qold,pold,q,v,f,data,mean);
